...
frequency_bands = [
    ...
    ("delta_alpha", True, True, False),
    ("delta_beta", True, False, True),
    ("alpha_beta", False, True, True),
    ("all", True, True, True)]
...
for subject in [1, 2]:
    for frequency_band in frequency_bands:
        ...
        #oneback x twoback
        data_zero_back = import_subject(subject, "oneback", delta=delta, alpha=alpha, beta=beta)
        data_one_back = import_subject(subject, "twoback", delta=delta, alpha=alpha, beta=beta)
        classify(data_zero_back, data_one_back, f"TwoClassSub{subject}_{band_name}_onebacktwoback")
        #oneback x zeroback
        data_zero_back = import_subject(subject, "oneback", delta=delta, alpha=alpha, beta=beta)
        data_one_back = import_subject(subject, "zeroback", delta=delta, alpha=alpha, beta=beta)
        classify(data_zero_back, data_one_back, f"TwoClassSub{subject}_{band_name}_onebackzeroback")
        #twoback x zeroback
        data_zero_back = import_subject(subject, "zeroback", delta=delta, alpha=alpha, beta=beta)
        data_one_back = import_subject(subject, "twoback", delta=delta, alpha=alpha, beta=beta)
        classify(data_zero_back, data_one_back, f"TwoClassSub{subject}_{band_name}_twobackzeroback")
